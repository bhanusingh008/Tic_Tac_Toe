...
class TicTacToeGameinSwing implements ActionListener
{
    ...
    JPanel t_panel = new JPanel(); // top_panel
    JPanel bt_panel = new JPanel(); // bottom panel
    ...
    TicTacToeGameinSwing()
    {
        ...
        textfield.setForeground(new Color(150,0,0));
        textfield.setFont(new Font("Serif", Font.BOLD, 35));
        textfield.setHorizontalAlignment(JLabel.CENTER);
        textfield.setOpaque(true);

        t_panel.setLayout(new BorderLayout());
        t_panel.setBounds(0, 0, 0, 0);
        ...
        for (int i = 0; i < 9; i++)
        {
            ...
            bton[i].addActionListener(this); // for the event of pressing this button.
            bton[i].setBackground(Color.LIGHT_GRAY);
        }

        t_panel.add(textfield);
        frame.add(t_panel, BorderLayout.SOUTH);
        frame.add(bt_panel);
    ...
    public void gameOver(String s) // what to do when game is over.
    {
        chance_flag = 0;
    ...
    public void actionPerformed(ActionEvent e)
    {
        for (int i = 0; i < 9; i++)
        {
            if (e.getSource() == bton[i])
            {
                if (pl1_chance)
                {
                    if (bton[i].getText() == "") // if button not pressed yet.
                    {
                        bton[i].setForeground(new Color(210, 10, 10));
                        bton[i].setText("X");
                        pl1_chance = false;
                ...
                else
                {
                    if (bton[i].getText() == "") // if button not pressed yet.
                    {
                        bton[i].setForeground(new Color(0, 100, 9));
                        bton[i].setText("O");
                        pl1_chance = true;
